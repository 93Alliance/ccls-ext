...
// 获得客户端配置，这个过程会解析用户设置的参数，然后生成一个 配置
...
export class ServerContext implements Disposable {
    ...
    public async start() {
        ...
        try {
            ...
            window.showErrorMessage(`Failed to start ccls with command "${this.cliConfig.launchCommand
                }".`);
        }
        // General commands.
    ...
    private displayCodeLens(document: TextDocument, allCodeLens: CodeLens[]) {
        ...
        for (const editor of window.visibleTextEditors) {
            ...
            for (const codeLens of allCodeLens) {
                ...
                // functions.
                if (codeLens.range.start.line !== codeLens.range.end.line) { position = new Position(codeLens.range.start.line, 1000000); }
